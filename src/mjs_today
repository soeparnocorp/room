...
//
// ===============================
// BINDINGS - TERSEDIA (opsional)
// ===============================
// - KV: READTALK_KV (untuk session & logs)
// - D1: READTALK_DB (untuk messages)
// - R2: READTALK_R2 (untuk archive)
...
async function handleApiRequest(path, request, env) {
  switch (path[0]) {
    case "room": {
      // 🔴 PRIVATE ROOM DIHAPUS - Harus ada nama room
      if (!path[1]) {
      ...
      // ✅ HANYA TERIMA PUBLIC ROOM (string, max 32 chars)
      if (name.length <= 32) {
        var id = env.rooms.idFromName(name);
      } else {
