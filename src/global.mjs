...
async function handleApiRequest(path, request, env) {
  // ===== CREATE BLOG ENDPOINT (BARU) =====
  if (path[0] === "create-blog" && request.method === "POST") {
    try {
      const { name, email, bio, avatar, deviceId } = await request.json();
...
Welcome **${name}** to READTalk!

${bio ? '**Bio:** ' + bio + '\n\n' : ''}
Email: ${email}

[💬 Start Chatting](https://account.soeparnocorp.workers.dev)
`;

      // Panggil GitHub API
      const githubRes = await fetch(
        `https://api.github.com/repos/${env.GIT_REPO_KEY}/contents/src/content/blog/${slug}.md`,
        {
          method: 'PUT',
          headers: {
            'Authorization': `token ${env.GIT_TOKEN_KEY}`,
      ...
      // Simpan juga ke KV (opsional)
      await env.READTALK_KV.put(`user:${deviceId}`, JSON.stringify({
        name,
        email,
        bio,
        avatar,
      ...
      return new Response(JSON.stringify({ 
        success: true, 
        url: `https://readtalk.pages.dev/blog/${slug}`,
