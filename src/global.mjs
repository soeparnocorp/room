...
async function handleApiRequest(path, request, env) {
  ...
  switch (path[0]) {
    case "room": {
      // CREATE PRIVATE ROOM TELAH DIHAPUS!
      // Hanya untuk akses room yang sudah ada
      let name = path[1];
      if (!name) {
...
export class ChatRoom {
  ...
  async fetch(request) {
    return await handleErrors(request, async () => {
      ...
      switch (url.pathname) {
        ...
        // ===== TAMBAH: Endpoint untuk daftar peserta (pencarian) =====
        case "/participants": {
          const participants = [];
  ...
  async webSocketMessage(webSocket, msg) {
    try {
      ...
      if (!session.name) {
        // First message: set name (MAX 12 KARAKTER - DIUBAH)
        session.name = "" + (data.name || "anonymous");
        webSocket.serializeAttachment({ ...webSocket.deserializeAttachment(), name: session.name });

        if (session.name.length > 12) {  // <-- UBAH DARI 32 KE 12
