...
async function handleApiRequest(path, request, env) {
  switch (path[0]) {
    ...
    // ===== NEW: USER API ENDPOINT UNTUK KV =====
    case "user": {
      const code = path[1]; // /api/user/:code
      
      // GET /api/user/:code - Ambil data user
      if (request.method === "GET") {
        ...
        try {
          ...
          const identitas = await env.USER_KV.get(`code:${code}`);
          ...
          const userData = await env.USER_KV.get(`user:${identitas}`);
      ...
      // POST /api/user/:code - Simpan data user baru
      if (request.method === "POST") {
        if (!code) {
          return new Response("Code required", { status: 400 });
        }
        ...
        try {
          ...
          // Cek apakah identitas sudah dipakai oleh code lain
      ...
      // PUT /api/user/:code - Update data user
      ...
      switch (url.pathname) {
        case "/websocket": {
          ...
          if (code) {
            ...
            // TODO: Validasi code dan ambil data user dari KV
            // Ini bisa ditambahkan nanti untuk autentikasi WebSocket
          }
          // ===== END =====
  ...
  async webSocketMessage(webSocket, msg) {
    try {
      ...
      if (!session.name) {
        ...
        // Simpan juga identitas dan code jika ada
        if (data.identitas) {
          session.identitas = data.identitas;
        }
        if (data.code) {
