...
async function handleApiRequest(path, request, env) {
  // ===== ENDPOINT CEK USER PAKE KV =====
  if (path[0] === "check-user") {
    ...
    try {
      // CEK KE KV APAKAH CODE ADA
      const userData = await env.READTALK_KV.get(`user:${code}`);
      
      if (userData) {
        // USER LAMA - parse dulu lalu kirim dengan format yang benar
  ...
  // ===== ENDPOINT SIMPAN USER BARU =====
  if (path[0] === "save-user" && request.method === "POST") {
    try {
      ...
      // SIMPAN KE KV
      await env.READTALK_KV.put(`user:${code}`, userData);
    ...
    return await handleErrors(request, async () => {
      ...
      switch (url.pathname) {
        case "/websocket": {
          ...
          const code = url.searchParams.get("code");
          if (code) {
            console.log("OAuth code detected in WebSocket:", code);
          }
          // ===== END =====
