...
async function handleApiRequest(path, request, env) {
  // ===== ENDPOINT BARU: SIMPAN USER KE KV =====
  if (path[0] === "user" && request.method === "POST") {
    try {
      ...
      // Simpan mapping email -> userId
      await env.READTALK_KV.put(`email:${email}`, userId, {
        expirationTtl: 60 * 60 * 24 * 365
      });
      
      // Cek apakah user sudah ada
      const existingUser = await env.READTALK_KV.get(`user:${userId}`);
      ...
      if (!existingUser) {
        ...
        await env.READTALK_KV.put(`user:${userId}`, JSON.stringify(newUser), {
          expirationTtl: 60 * 60 * 24 * 365
        });
        ...
        await env.READTALK_KV.put(`user:${userId}`, JSON.stringify(userData), {
          expirationTtl: 60 * 60 * 24 * 365
        });
        
        return new Response(JSON.stringify({ success: true, isNew: false }), {
          headers: { "Content-Type": "application/json" }
        });
      }
    } catch (err) {
  ...
  // ===== ENDPOINT BARU: AMBIL USER DARI KV =====
  if (path[0] === "user" && request.method === "GET") {
    try {
      ...
      if (!targetId && email) {
        targetId = await env.READTALK_KV.get(`email:${email}`);
      }
      ...
      const userData = await env.READTALK_KV.get(`user:${targetId}`);
...
// ===== CHATROOM & RATELIMITER CLASSES (SAMA PERSIS) =====
    ...
    return await handleErrors(request, async () => {
      ...
      switch (url.pathname) {
        case "/websocket": {
          ...
          const email = url.searchParams.get("email");
          if (code) console.log("OAuth code detected:", code);
          if (email) console.log("Email detected:", email);
